* Estimate results for Supplemental eTable 5: Support for alcohol control policies by reading the current alcohol health warning in the past 30 days 
...
* Calculate weighted support for people who didn't read label 

foreach var in `polvars' {
    svy: proportion `var', subpop(if yesusedlabel == 0)
 }

* Calculate weighted support for people who read label 

foreach var in `polvars' {
    svy: proportion `var', subpop(if yesusedlabel == 1)
}
